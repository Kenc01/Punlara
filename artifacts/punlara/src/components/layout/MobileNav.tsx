...
export default function MobileNav() {
  ...
  if (isAuthenticated) {
    navItems.push({ href: "/my-tree", icon: "dashboard", label: "My Tree" });
  }
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className="flex flex-col items-center justify-center w-16 cursor-pointer" data-testid={`link-mobilenav-${item.label.toLowerCase()}`}>
                <span 
